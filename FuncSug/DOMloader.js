...
async function getScripts() {
	...
	//~ console.log(theScripts)
	const scriptPromises = Array.from(theScripts)
		.map(
			//~ scriptElt   =>   (scriptElt.getAttribute('src')) ? fetch(scriptElt.getAttribute('src')) : scriptElt.innerHTML
			scriptElt   =>   (scriptElt.getAttribute('src')) ? fetch(scriptElt.getAttribute('src')) : scriptElt.childNodes[0].nodeValue
		)
	//~ console.log(scriptPromises)
	const values = await Promise.all(scriptPromises)
	//~ console.log(values)
	//~ const valPromises =    values.map(  val  =>  (val.text())  )
	const valPromises =    values.map(  val  =>  (val.text) ? val.text() : val )
	//~ console.log(valPromises)
